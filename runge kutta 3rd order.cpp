// runga_kutta_3rd_order.cpp : Defines the entry point for the console application.
//
...
double k3(double y0, double x0, double interval) {
	...
	double y = y0 - k1(y0, x0, interval) + 2*k2(y0 , x0 , interval) ;
	double result = interval * function(y, x);
	return result;
}
double euler_result(double y0, double x0, double xn, double interval) {
	double result = 0.0;
	while (x0 != xn) {
		result = y0 + (k1(y0, x0, interval) + 4*k2(y0, x0, interval) + k3(y0 , x0 , interval)) / 6;
